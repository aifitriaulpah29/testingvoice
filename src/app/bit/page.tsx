...
function BitPageContent() {
  const searchParams = useSearchParams();
  const id = searchParams.get('id') || '';
  const mode = 'play'; // Mode play untuk halaman /bit
  ...
  const buildUI = useCallback(() => {
    ...
    return new bitmovin.playerui.UIContainer({
      ...
      hideDelay: 2000,
      // FIX: Hapus 'Prepared' dari exceptions — jika player stuck di state ini,
      // kontrol akan bisa auto-hide normal. Hanya Paused dan Finished yang relevan.
    ...
    const init = async () => {
      try {
        ...
        if (configObj) {
          ...
          // FIX: Fetch token jika proxy aktif ATAU jika ada origin/referer yang membutuhkan bypass.
          // Ini selaras dengan kondisi shouldProxy di applyCustomHeaders.
          const needsProxy = configObj.proxy === true ||
                             !!(configObj.origin && configObj.origin.trim()) ||
                             !!(configObj.referer && configObj.referer.trim());

          if (needsProxy) {
            try {
              const tokenRes = await fetch(`/api/token?id=${encodeURIComponent(configObj.id)}`);
              if (tokenRes.ok) {
          ...
          // If it's a raw base64 URL, we sign it by passing url parameter
          try {
            const tokenRes = await fetch(`/api/token?url=${encodeURIComponent(id)}`);
            if (tokenRes.ok) {
              const tokenData = await tokenRes.json();
              token = tokenData.token;
            }
          } catch (tokenErr) {
            console.error('Gagal mengambil token streaming untuk url raw:', tokenErr);
          }
        }
        ...
        // Local function to apply custom headers/proxy configuration in Bitmovin
        const applyCustomHeadersLocal = (config: any, tokenStr: string) => {
          return {
            preprocessHttpRequest: function(type: any, request: any) {
              const cfg = config || { proxy: true, id: '', url: streamUrl };
              const shouldProxy = cfg.proxy === true || !!tokenStr;
              ...
              if (shouldProxy) {
                ...
                if (targetUrl.startsWith(localApiPrefix)) {
                  ...
                  const baseCdnUrl = cfg.url.substring(0, cfg.url.lastIndexOf('/') + 1);
            ...
            preprocessHttpResponse: function(type: any, response: any) {
              return response;
            },
            httpHeaders: {
        ...
        const playerConfig = {
          ...
          network: applyCustomHeadersLocal(configObj, token)
        };
        ...
        // Mengandalkan autoplay: true saja tidak cukup di browser modern —
        // ini memastikan poster menghilang dan video mulai berjalan.
        try {
          await player.play();
        } catch (playErr) {
          // Autoplay mungkin diblokir — user bisa klik play manual
          console.warn('Autoplay diblokir browser, menunggu interaksi user:', playErr);
        }
